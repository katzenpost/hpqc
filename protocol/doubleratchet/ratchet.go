...
func (s *state) Upgrade(scheme *hybrid.Scheme) error {
	...
	util.ExplicitBzero(s.SendPQRatchetPrivate)
	util.ExplicitBzero(s.RecvPQRatchetPublic)
...
func (r *Ratchet) trySavedKeys(ciphertext []byte) ([]byte, error) {
	...
	for headerKey, messageKeys := range r.saved {
		...
		util.ExplicitBzero(msgKey.key)
		if len(messageKeys) == 0 {
			delete(r.saved, headerKey)
			util.ExplicitBzero(headerKey[:])
		}
		return msg, nil
	}
	...
	for headerKey, newMessageKeys := range newKeys {
		...
		if ok {
			...
			for _, messageKey := range newMessageKeys {
				util.ExplicitBzero(messageKey.key)
			}
		} else {
...
func (r *Ratchet) wipeSavedKeys() {
	for headerKey, keys := range r.saved {
		for _, savedKey := range keys {
			util.ExplicitBzero(savedKey.key)
		}
		delete(r.saved, headerKey)
		util.ExplicitBzero(headerKey[:])
	}
...
func (r *Ratchet) Decrypt(ciphertext []byte) ([]byte, error) {
	...
	ok = ok && !util.CtIsZero(r.recvHeaderKey)
...
func DestroyRatchet(r *Ratchet) {
	util.ExplicitBzero(r.rootKey)
	util.ExplicitBzero(r.sendHeaderKey)
	util.ExplicitBzero(r.recvHeaderKey)
	util.ExplicitBzero(r.nextSendHeaderKey)
	util.ExplicitBzero(r.nextRecvHeaderKey)
	util.ExplicitBzero(r.sendChainKey)
	util.ExplicitBzero(r.recvChainKey)
	r.sendRatchetPrivate.Reset()
